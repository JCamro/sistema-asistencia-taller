...
function ProfesoresPage() {
  ...
  return (<div style={{maxWidth:'1100px',margin:'0 auto'}}>
    ...
    {ho&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50}}><div style={{background:'white',borderRadius:'16px',width:'100%',maxWidth:'600px',maxHeight:'80vh',overflow:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.15)'}}><div style={{padding:'1.25rem 1.5rem',borderBottom:'1px solid #f3f4f6',display:'flex',justifyContent:'space-between',alignItems:'center'}}><h2 style={{fontSize:'1.125rem',fontWeight:700,color:'var(--color-bg-dark)',margin:0}}>Historial de Pagos</h2><button onClick={()=>setHo(false)} style={{width:32,height:32,borderRadius:'50%',border:'none',background:'#f3f4f6',color:'#6b7280',fontSize:'1.25rem',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>×</button></div><div style={{padding:'1.5rem'}}>{hl?<p style={{textAlign:'center',color:'var(--color-text-muted)',padding:'2rem'}}>Cargando...</p>:hp.length===0?<p style={{textAlign:'center',color:'var(--color-text-muted)',padding:'2rem'}}>No hay pagos</p>:<div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:mb?450:'auto'}}><thead><tr style={{background:'#f8fafc',borderBottom:'1.5px solid #f1f5f9'}}><th style={{padding:'0.5rem 0.75rem',textAlign:'left',fontSize:'0.65rem',fontWeight:600,color:'var(--color-text-muted)',textTransform:'uppercase'}}>Fecha</th><th style={{padding:'0.5rem 0.75rem',textAlign:'right',fontSize:'0.65rem',fontWeight:600,color:'var(--color-text-muted)',textTransform:'uppercase'}}>Monto</th><th style={{padding:'0.5rem 0.75rem',textAlign:'left',fontSize:'0.65rem',fontWeight:600,color:'var(--color-text-muted)',textTransform:'uppercase'}}>Descripción</th><th style={{padding:'0.5rem 0.75rem',textAlign:'center',fontSize:'0.65rem',fontWeight:600,color:'var(--color-text-muted)',textTransform:'uppercase'}}>Estado</th></tr></thead><tbody>{hp.map((pg:any)=>(<tr key={pg.id} style={{borderBottom:'1px solid #f8fafc'}}><td style={{padding:'0.5rem 0.75rem',fontSize:'0.8125rem'}}>{new Date(pg.fecha).toLocaleDateString('es-PE',{day:'numeric',month:'short',year:'numeric'})}</td><td style={{padding:'0.5rem 0.75rem',textAlign:'right',fontWeight:600,fontFamily:'monospace'}}>{formatMonto(pg.monto)}</td><td style={{padding:'0.5rem 0.75rem',fontSize:'0.8125rem',color:'#64748b'}}>{pg.descripcion||'—'}</td><td style={{padding:'0.5rem 0.75rem',textAlign:'center'}}><span style={{padding:'0.15rem 0.5rem',borderRadius:'9999px',fontSize:'0.7rem',fontWeight:600,background:pg.estado==='pendiente'?'#fef3c7':'#ecfdf5',color:pg.estado==='pendiente'?'#d97706':'var(--color-success)'}}>{pg.estado_display}</span></td></tr>))}</tbody></table></div>}<Button type="button" variant="secondary" onClick={()=>setHo(false)} style={{width:'100%',marginTop:'1rem'}}>Cerrar</Button></div></div></div>)}
  </div>);
}
