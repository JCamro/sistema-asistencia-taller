...
function PanelLateral({ horario, estaLleno, onClose, onActualizar }: { horario: Horario; estaLleno: boolean; onClose: () => void; onActualizar: (c:number,o:number)=>void }) {
  ...
  const [editando, setEditando] = useState(false); const [cv, setCv] = useState(horario.cupo_maximo.toString()); const [g, setG] = useState(false);
  const guardar = async () => { const n=parseInt(cv); const o=horario.ocupacion??0; if(n<o){showToast(`No se puede reducir por debajo de ${o} alumno(s)`,'warning');return} setG(true);
    try { const r=await fetch(`${apiBase}/api/horarios/${horario.id}/`,{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:`Bearer ${localStorage.getItem('access_token')}`},body:JSON.stringify({cupo_maximo:n})}); if(!r.ok)throw new Error(JSON.stringify(await r.json())); showToast('Cupo actualizado','success');setEditando(false);onActualizar(n,o); } catch{showToast('Error','error')} finally{setG(false)} };
  return (<div style={{ background:'white',borderRadius:'14px',border:'1px solid #f1f5f9',padding:'1.25rem',position:'sticky',top:'1rem' }}>
    ...
    <div style={{ display:'flex',flexDirection:'column',gap:'0.5rem',marginBottom:'1rem' }}>
      ...
      <div style={{ background:estaLleno?'#fef2f2':'#f0fdf4',borderRadius:'10px',padding:'0.75rem',border:estaLleno?'1px solid #fecaca':'1px solid #bbf7d0' }}>
        ...
        {editando?<div style={{ marginTop:'0.5rem' }}><input type="number" value={cv} onChange={e=>setCv(e.target.value)} min={horario.ocupacion??1} style={{ width:'100%',padding:'0.5rem',border:'1px solid #e5e7eb',borderRadius:'8px',fontSize:'0.8125rem',marginBottom:'0.25rem' }}/><div style={{ fontSize:'0.65rem',color:'#94a3b8',marginBottom:'0.5rem' }}>Mín: {horario.ocupacion??0} alumnos</div><div style={{ display:'flex',gap:'0.375rem' }}><button onClick={()=>setEditando(false)} style={btnSm}>Cancelar</button><button onClick={guardar} disabled={g||!cv||parseInt(cv)<(horario.ocupacion??0)} style={{ ...btnSm,background:g?'#e5e7eb':'#d4af37',color:g?'#9ca3af':'#0a0a0a' }}>{g?'...':'Guardar'}</button></div></div>:<div style={{ display:'flex',alignItems:'baseline',gap:'0.25rem',marginTop:2 }}><span style={{ fontSize:'1.25rem',fontWeight:800,color:estaLleno?'#dc2626':'#059669' }}>{horario.ocupacion??0}</span><span style={{ fontSize:'0.8125rem',color:'#94a3b8' }}>/ {horario.cupo_maximo}</span>{estaLleno&&<span style={{ marginLeft:'auto',fontSize:'0.6rem',fontWeight:700,background:'#dc2626',color:'white',padding:'0.125rem 0.45rem',borderRadius:4 }}>LLENO</span>}</div>}
      </div>
    </div>
...
function HorariosPage() {
  ...
  const cargarTalleres = useCallback(async()=>{if(isCicloLoading||!cicloActual)return;try{const r=await fetch(`${apiBase}/api/ciclos/${cicloActual.id}/talleres/`,{headers:{Authorization:`Bearer ${localStorage.getItem('access_token')}`}});setTalleres(((await r.json()).results))}catch(err){showApiError(err)}finally{setLt(false)}},[cicloActual,isCicloLoading,showApiError]);
  const cargarHorarios = useCallback(async(tId:number)=>{if(!cicloActual)return;setLh(true);    try{const r=await fetch(`${apiBase}/api/horarios/?taller=${tId}&page_size=100`,{headers:{Authorization:`Bearer ${localStorage.getItem('access_token')}`}});const d=await r.json();setHorarios(d.results||d||[])}catch(err){showApiError(err)}finally{setLh(false)}},[cicloActual,showApiError]);
  ...
  const gridHorarios = useMemo(()=>{const g:Record<string,Horario>={};horarios.forEach(h=>{const i=horaAIndice(h.hora_inicio);if(i!==-1)g[`${h.dia_semana}-${HORAS[i]}`]=h});return g},[horarios]);
        ...
        <div style={{background:'white',borderRadius:'14px',border:'1px solid #f1f5f9',overflow:'hidden'}}>
          ...
          :<div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
            ...
            {HORAS.map(hora=>(<div key={hora} style={{display:'grid',gridTemplateColumns:'48px repeat(7,1fr)',borderBottom:'1px solid #f8fafc',minHeight:56}}><div style={{padding:'0.35rem',fontSize:'0.65rem',color:'#cbd5e1',textAlign:'center',borderRight:'1px solid #f8fafc',display:'flex',alignItems:'center',justifyContent:'center'}}>{formatHora(hora)}</div>
              {DIAS.map(dia=>{const key=`${dia.value}-${hora}`;const h=gridHorarios[key];const lleno=h?(h.ocupacion??0)>=h.cupo_maximo:false;
                return(<div key={key} style={{borderLeft:'1px solid #f8fafc',padding:2}}>{h&&<CeldaCalendario horario={h} estaLleno={lleno} isSelected={hs?.id===h.id} onClick={()=>setHs(hs?.id===h.id?null:h)}/>}</div>);})}
            </div>))}
          </div>}
        </div>
      </div>
