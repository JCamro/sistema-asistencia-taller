...
function PanelLateral({ horario, estaLleno, onClose, onActualizar }: { horario: Horario; estaLleno: boolean; onClose: () => void; onActualizar: (c:number,o:number)=>void }) {
  ...
  const guardar = async () => { const cupoInt=parseInt(cupoValor); const ocupacion=horario.ocupacion??0; if(cupoInt<ocupacion){showToast(`No se puede reducir por debajo de ${ocupacion} alumno(s)`,'warning');return} setG(true);
    try { const response=await fetch(`${apiBase}/horarios/${horario.id}/`,{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:`Bearer ${localStorage.getItem('access_token')}`},body:JSON.stringify({cupo_maximo:cupoInt})}); if(!response.ok)throw new Error(JSON.stringify(await response.json())); showToast('Cupo actualizado','success');setEditando(false);onActualizar(cupoInt,ocupacion); } catch{showToast('Error','error')} finally{setG(false)} };
  return (<div style={{ background:'white',borderRadius:'14px',border:'1.5px solid #c8ccd4',padding:'1.25rem',position:'sticky',top:'1rem' }}>
    <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem' }}><span style={{ fontSize:'0.875rem',fontWeight:600,color:'#0f172a' }}>Detalle de clase</span><button onClick={onClose} style={{ width:28,height:28,borderRadius:'50%',border:'none',background:'#e5e7eb',color:'#6b7280',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>×</button></div>
    <div style={{ display:'flex',flexDirection:'column',gap:'0.5rem',marginBottom:'1rem' }}>
      <div style={{ background:'#fafbfc',borderRadius:'10px',padding:'0.75rem',border:'1.5px solid #c8ccd4' }}><span style={sc}>Día y hora</span><span style={{ ...sv,display:'block',marginTop:1 }}>{diaLabel} · {normalizarHora(horario.hora_inicio)} – {normalizarHora(horario.hora_fin)}</span></div>
      <div style={{ background:'#fafbfc',borderRadius:'10px',padding:'0.75rem',border:'1.5px solid #c8ccd4' }}><span style={sc}>Profesor</span><span style={{ ...sv,display:'block',marginTop:1 }}>{horario.profesor_nombre}</span></div>
    ...
    {ts&&(<div style={{display:'flex',gap:'1rem',flexWrap:'wrap'}}>
      <div style={{flex:mb?'1 1 100%':hs?7:1,minWidth:0,width:mb?'100%':undefined}}>
        <div style={{background:'white',borderRadius:'14px',border:'1.5px solid #c8ccd4',overflow:'hidden'}}>
          ...
          {lh?<div style={{display:'flex',justifyContent:'center',padding:'3rem'}}><div style={{width:32,height:32,border:'3px solid #e2e8f0',borderTop:'3px solid #d4af37',borderRadius:'50%',animation:'spin 0.8s linear infinite'}}/></div>
          ...
          :<div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
            <div style={{display:'grid',gridTemplateColumns:'48px repeat(7,1fr)',background:'#fafbfc',borderBottom:'1px solid #e2e8f0'}}><div style={{padding:'0.5rem',fontSize:'0.65rem',fontWeight:600,color:'#94a3b8',textAlign:'center',textTransform:'uppercase',letterSpacing:'0.04em'}}></div>{DIAS.map(d=><div key={d.value} style={{padding:'0.5rem 0.25rem',fontSize:'0.65rem',fontWeight:600,color:'#94a3b8',textAlign:'center',letterSpacing:'0.04em'}}>{d.abrev}</div>)}</div>
            {HORAS.map(hora=>(<div key={hora} style={{display:'grid',gridTemplateColumns:'48px repeat(7,1fr)',borderBottom:'1px solid #f8fafc',minHeight:56}}><div style={{padding:'0.35rem',fontSize:'0.65rem',color:'#cbd5e1',textAlign:'center',borderRight:'1px solid #f0f2f5',display:'flex',alignItems:'center',justifyContent:'center'}}>{formatHora(hora)}</div>
              {DIAS.map(dia=>{const key=`${dia.value}-${hora}`;const horario=gridHorarios[key];const lleno=horario?(horario.ocupacion??0)>=horario.cupo_maximo:false;
                return(<div key={key} style={{borderLeft:'1px solid #f0f2f5',padding:2}}>{horario&&<CeldaCalendario horario={horario} estaLleno={lleno} isSelected={hs?.id===horario.id} onClick={()=>setHs(hs?.id===horario.id?null:horario)}/>}</div>);})}
            </div>))}
          </div>}
        </div>
      </div>
