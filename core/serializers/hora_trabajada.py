...
class HoraTrabajadaListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'estado', 'estado_display', 'created_from', 'created_from_display',
            'num_alumnos', 'valor_generado', 'monto_profesor',
        ]
...
class HoraTrabajadaCreateUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = HoraTrabajada
        fields = [
            'profesor', 'ciclo', 'horario', 'fecha', 'tipo',
            'horas_trabajadas', 'num_alumnos', 'valor_generado',
            'monto_base', 'monto_adicional', 'monto_profesor',
            'ganancia_taller', 'observacion',
        ]
    ...
    def validate(self, data):
        ...
        # Validar horario: obligatorio excepto para hora_extra
        if tipo != 'hora_extra' and not data.get('horario'):
            raise serializers.ValidationError(
                {"horario": "El horario es obligatorio para este tipo de registro."}
            )

        if tipo == 'hora_extra' and data.get('horario'):
            raise serializers.ValidationError(
                {"horario": "Hora extra no debe tener horario asociado."}
            )
